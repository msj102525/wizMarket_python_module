    ...
    try:
        ...
        for _, row in df.iterrows():
            ...
            if current_store_business_number in store_business_number_set:
                update_data_to_new_local_store(new_data)
                update_new_count += 1
            else:
                insert_data_to_new_local_store(new_data)
                insert_new_count += 1

        return (file_path, update_new_count, insert_new_count)
...
def process_csv_files_parallel(year, quarter):
    ...
    with ProcessPoolExecutor(max_workers=5) as executor:
        futures = {executor.submit(process_single_csv_file, file, year, quarter, store_business_number_set, cities, districts, sub_districts): file for file in files}

        for future in as_completed(futures):
            try:
                processed_store_numbers, file_update_count, file_insert_count = future.result()
    ...
    # 3. 존재하지 않는 store_business_number 업데이트 (병렬 처리 후)
    update_old_count = 0
    connection = get_db_connection()
    ...
    try:
        ...
        print(f"Error updating missing store numbers: {e}")
        rollback(connection)
    finally:
        close_connection(connection)
